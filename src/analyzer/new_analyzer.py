...
def cmp_atomic_attack(a: AtomicAttack, b: AtomicAttack):
    impact = cmp_impact(a.impact, b.impact)
    if impact == 0:
        if a.score > b.score:
            return 1
...
class ModelAnalyzer:
    ...
    def analyze_status(self, model: nx.DiGraph) -> nx.DiGraph:
        ...
        for node_name, node_prop in model.nodes(data=True):
            ...
            #     edges.append((node_name, f"{node_name}:{prop}", {'weight': 0, 'score': 0}))
            #     edges.append((f"{node_name}:{prop}", node_name, {'weight': 0, 'score': 0}))
            for trans in internal_transitions:
                edges.append((f"{node_name}:{trans[0]}", f"{node_name}:{trans[1]}", {'weight': 0, 'score': 0}))
            atomic_attacks: dict[str, AtomicAttack] = node_prop['atomic_attacks']
            for src_name, _, edge_prop in model.in_edges(node_name, data=True):
                ...
                edges.append((src_status, dst_status, {'weight': 0, 'score': atomic_attacks[access].score, 'exploit_name': atomic_attacks[access].id}))
        new_model.add_edges_from(edges)
        # pos = nx.spring_layout(model)
        # nx.draw(model, pos, with_labels = True, node_size = 300)
        # plt.show()
        return new_model
    ...
    def analyze_vul(self, model: nx.DiGraph):
        for node_name, node_prop in model.nodes(data=True):
            ...
            for atomic in node_prop["atomic"]:
            ...
            node_prop["atomic_attacks"] = classified_atomic_attacks
